...
class Component extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    /*if( type === 'CONFIRMED_SUCCESS') {
      this.setState({sum: result.indonesia.sum, newcase: result.indonesia.newcase, detail: result.indonesia.detail})
    }*/
  }
  ...
  render() {
    ...
    return (
      <div>
        {indonesia !== null &&
          <>
            <Typography color="textSecondary" className={clsx(classes.body2, classes.title)}>Terkonfirmasi</Typography>
            <Typography className={clsx(classes.h2, classes.bold)}>{indonesia.TotalCases}</Typography>
            <Typography className={clsx(classes.overline, classes.newcase)}>({indonesia.NewCases})</Typography>
          </>
        }
        {/* sum !== null &&
